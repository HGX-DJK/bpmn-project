<template>
    ...
    <template #title>
      <collapse-title title="执行监听">
        <span class="iconfont icon-jianting"></span>
      </collapse-title>
    ...
    <div class="element-extension-listeners">
      ...
      <button  class="inline-large-button"  @click="openListenerModel(-1)">添加执行监听</button>
    </div>
    <!--执行监听属性面板-->
    <el-dialog v-model="modelVisible" title="添加执行监听器" width="500px" append-to-body destroy-on-close>
...
import { addExecutionListener,getExecutionListeners, getDefaultEvent,  getExecutionListenerType,getExecutionListenerTypes,updateExecutionListener,removeExecutionListener} from "@/api/panel-util/executionListenersUtil";
    ...
    setup() {
        ...
        const listener = reactive({
            ...
            reloadExtensionListeners() {
                ...
                /**获取监听事件类型 */
                listener.listenerEventTypeOptions = getExecutionListenerTypes(getActive());
                listener.listenersRaw = getExecutionListeners(getActive());
            ...
            async saveExecutionListener(index) {
                ...
                listener.activeIndex === -1
                    ? addExecutionListener(getActive(), listener.newListener)
                    : updateExecutionListener(getActive(), listener.newListener, listener.listenersRaw[listener.activeIndex]);
                listener.reloadExtensionListeners();
            },
            /**移除监听项 */
            removeListener(index) {
                const listenerItem = listener.listenersRaw[index];
                removeExecutionListener(getActive(), listenerItem);
                listener.reloadExtensionListeners();
            },
            /**更新监听类型 */
